...
Los **conjuntos** son mutables (puedes añadir o eliminar elementos) y se definen utilizando llaves `{}` (similar a los [[Dictionaries|diccionarios]], pero sin pares clave-valor) o la función `set()`.
...
1. **Inicialización:** Puedes crear un conjunto a partir de una [[Lists and Tuples#Listas|lista]], [[Lists and Tuples#Tuplas|tupla]] o directamente con llaves.
...
3. **Manejo de Índices:** Los conjuntos son desordenados, lo que significa que no mantienen un orden específico de los elementos y, por lo tanto, no soportan [[Lists and Tuples#Índices y Slicing (_Rebanado_)|indexación ni slicing]] como las listas o tuplas.
...
- `set.remove(elemento)`: Elimina el elemento especificado del conjunto. Si el elemento no existe, lanza un [[Exception Handling#Excepciones Comunes|KeyError]].
...
- **`set.union(otro_set, ...)` / Operador `|`:** (Similar a [[Dictionaries#Unión de Diccionarios|unión de diccionarios]]) Devuelve un nuevo conjunto con todos los elementos de ambos (o más) conjuntos, sin duplicados.
